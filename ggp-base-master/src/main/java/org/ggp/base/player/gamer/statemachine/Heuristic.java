...
public class Heuristic extends the_men_who_stare_at_goats {
 ...
 */
	...
	protected int alphabeta(MachineState state, int alpha, int beta, int depth) throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
		if (machine.isTerminal(state)) return machine.getGoal(state, roles.get(self_index));
		if (depth == 0) return 1;

		List<Move> moves = machine.getLegalMoves(state, roles.get(self_index));
		for (Move m : moves) {
			List<List<Move>> jointMoves = machine.getLegalJointMoves(state, roles.get(self_index), m);
			int minValue = beta;
	...
	Pair<Integer, Move> miniMax(MachineState state, List<Move> moves, int depth) throws GoalDefinitionException, MoveDefinitionException, TransitionDefinitionException {
		Move maxMove = null;
		int alpha = MIN_GOAL - 1;

		for (Move m : moves) {
			if (System.currentTimeMillis() > finishBy) return null;
			List<List<Move>> jointMoves = machine.getLegalJointMoves(state, roles.get(self_index), m);

			int minValue = MAX_GOAL + 1;
			for (List<Move> jointMove: jointMoves) {
				if (System.currentTimeMillis() > finishBy) return null;
				MachineState nextState = machine.getNextState(state, jointMove);
				int result = alphabeta(nextState, alpha, MAX_GOAL + 1, depth);
				if (result <= alpha) {
					minValue = MIN_GOAL - 1;
					break;
				}
			...
			if (minValue == MAX_GOAL) return Pair.of(MAX_GOAL, m);
			else if (minValue > alpha) {
				alpha = minValue;
				maxMove = m;
			}
		}
		return Pair.of(alpha, maxMove);
	}
/*
 * FIND BEST MOVE
 */
	protected Move bestmove()
		...
		while (depth < maxdepth) {
			...
			Pair<Integer, Move> bestValMove = miniMax(state, moves, depth);
			if (System.currentTimeMillis() > finishBy) break;
			System.out.println(bestValMove);
			bestValue = bestValMove.left;
			bestMove = bestValMove.right;
			if (bestValue == MAX_GOAL) return bestMove;
		}
